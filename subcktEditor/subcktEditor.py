#!/usr/bin/python
# subcktEditor.py is a python script to create a fornt for subcircuit editor. It developed for FreeEDA software. It is written by Yogesh Dilip Save (yogessave@gmail.com).  
...
def importEditor(e=None):
  ...
  list=[".bak",".cir",".cir.out",".cir.ckt",".pro",".sub",".sch"]
  for i in list:
    if shutil.copy2(FreeEDA_HOME+"/subcktLibrary/"+subckt.Name+i,"."):
      tkMessageBox.showerror("Export Failed","Unable to export subcircuit file "+subckt.Name)
    else:
      pass
  #if shutil.copy2(FreeEDA_HOME+"/subcktLibrary/"+subckt.Name+".*","."):
...
def exportEditor(e=None):
  ...
  list=[".bak",".cir",".cir.out",".cir.ckt",".pro",".sub",".sch"]
  #if shutil.copy2(subckt.Name+".sch",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".bak",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".sub",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".pro",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".cir.ckt",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".cir.out",FreeEDA_HOME+"/subcktLibrary/") && shutil.copy2(subckt.Name+".cir",FreeEDA_HOME+"/subcktLibrary/"):
  for i in list: 
    if shutil.copy2(subckt.Name+i,FreeEDA_HOME+"/subcktLibrary/"):
      tkMessageBox.showerror("Export Failed","Unable to export subcircuit file "+subckt.Name)
    else:
      pass
